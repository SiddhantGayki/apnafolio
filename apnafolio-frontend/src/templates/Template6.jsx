...
export default function Template6({ data = dummyResume }) {
  ...
  <meta property="og:image" content={data?.contact?.photo || "/default-avatar.png"} />
      ...
      <header className="t7-navbar">
        ...
        <button className="theme-toggle" onClick={() => setTheme(theme === "light" ? "dark" : "light")}>{theme === "light" ? "🌙" : "☀️"}</button>
      </header>

      {/* Hero */}
      <section id="hero" className="t7-hero">
        <div className="t7-hero-bg"></div>
        <motion.img src={data?.contact?.photo} alt={data?.name} className="t7-avatar" animate={{ y: [0, -20, 0] }} transition={{ repeat: Infinity, duration: 4 }} />
        <motion.h1 className="t7-name" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>{data?.name}</motion.h1>
        <motion.h2 className="t7-role" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>{data?.role}</motion.h2>
        <p className="t7-summary">{data?.summary}</p>
      </section>

      {/* About */}
      <section id="about" className="t7-section">
        <h3>About Me</h3>
        <p>{data?.summary}</p>
      </section>
                ...
                {arr.map((s, i) => {
                  ...
                  return (
                    <motion.div key={i} className="t7-skill" whileHover={{ scale: 1.1 }}>
                      <img src={iconPath} alt={s} onError={(e) => e.currentTarget.src = "/icons/default.png"} />
                      <span>{s}</span>
                    </motion.div>
                  );
                })}
        ...
        <div className="t7-projects">
          {(data?.projects || []).map((p, i) => (
            <motion.div key={i} className="t7-project-card" whileHover={{ y: -5 }}>
              {p.video ? <video src={p.video} controls className="t7-project-media" /> : p.image && <img src={p.image} alt={p.title} className="t7-project-media" />}
              <h4>{p.title}</h4>
              <p>{p.description}</p>
              <div className="t7-tags">{(p.tags || []).map((t, idx) => <span key={idx} className="t7-tag">{t}</span>)}</div>
              <div className="t7-links">
                {p.link && <a href={p.link} target="_blank" rel="noreferrer">Live</a>}
                {p.document && <a href={p.document} download>Download</a>}
              </div>
            </motion.div>
          ))}
        </div>
      ...
      {/* Education */}
      <section id="education" className="t7-section">
        <h3>Education</h3>
      ...
      <section id="resume" className="t7-section">
        <h3>Resume</h3>
        {data?.resumeFile ? <a href={data.resumeFile} download className="t7-btn">Download Resume</a> : <p>No resume uploaded.</p>}
      </section>
      ...
      {/* Footer */}
      <footer className="t7-footer">© {new Date().getFullYear()} {data?.name} • Built with ApnaFolio</footer>
    </div>
  );
}
