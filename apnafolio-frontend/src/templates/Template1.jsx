  ...
  return (
    <div className="tpl2-glass">
      ...
      <section id="hero" className="tpl2-hero">
        <motion.div className="hero-card" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          <img
                ...
                <div className="skill-items">
                  {arr.map((s, idx) => (
                    <motion.div key={idx} className="skill-pill" whileHover={{ scale: 1.1 }}>
                      <img src={iconFor(s)} alt={s} onError={(e) => (e.currentTarget.src = "/icons/default.png")} />
                      <span>{s}</span>
                    </motion.div>
                  ))}
                </div>
            ...
            {d.projects.map((p, i) => (
              <motion.div key={i} className="card" whileHover={{ y: -6 }}>
                ...
                <div className="proj-links">
                  ...
                  {p.document && <a href={p.document} target="_blank" rel="noreferrer">Doc</a>}
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <section id="experience" className="tpl2-section">
          ...
          {d.experience.map((ex, i) => (
            <div key={i} className="card">
              ...
              {ex.document && <a href={ex.document} target="_blank" rel="noreferrer">Proof</a>}
            </div>
          ))}
        </section>
      )}
      ...
      {d.certifications?.length > 0 && (
        <section id="certifications" className="tpl2-section">
          ...
          {d.certifications.map((c, i) => (
            <div key={i} className="card">
              <p>{c.name}</p>
              {c.document && <a href={c.document} target="_blank" rel="noreferrer">View</a>}
            </div>
          ))}
        </section>
      )}
      ...
      <section id="resume" className="tpl2-section">
        <h2>Resume</h2>
        {d.resumeFile ? (
          <a href={`${d.resumeFile}?fl_attachment`} target="_blank" rel="noreferrer" className="t3-btn">
            Download Resume
          </a>
        ) : (
