...
export default function TemplateSwitcher({ current }) {
  ...
  const switchTemplate = async (tpl) => {
    if (tpl.id === current) return;
    try {
      setLoading(true); setMsg("");
      if (tpl.price === 0) {
        await UserAPI.switchTemplate(tpl.id);
        setMsg(`✅ Switched to ${tpl.name}`);
        return;
      }
      const orderRes = await PaymentAPI.createOrder(tpl.price);
      await loadRazorpay(orderRes.data.order, tpl.id);
      setMsg(`✅ Switched to ${tpl.name}`);
    } catch (err) {
  ...
  return (
    <div className="template-switcher-container">
      <div className="template-scroll-grid">
        {templates.map((tpl) => (
          <div key={tpl.id} className={`tpl-card-new ${tpl.id === current ? "active" : ""}`}>
            <div className="tpl-info">
              <h4>{tpl.name}</h4>
              <p className="tpl-price">{tpl.price === 0 ? "FREE" : `₹${tpl.price}`}</p>
            </div>
            <button
              className={`tpl-btn ${tpl.id === current ? "btn-active" : ""}`}
              onClick={() => switchTemplate(tpl)}
              disabled={loading || tpl.id === current}
            >
              {tpl.id === current ? "Active" : loading ? "..." : "Get"}
            </button>
          </div>
        ))}
      </div>
      {msg && <div className={`status-toast ${msg.includes('❌') ? 'error' : 'success'}`}>{msg}</div>}
    </div>
  );
}
// import { useState } from "react";
